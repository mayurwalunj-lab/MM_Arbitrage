...
let CONFIG = {
    ...
    // API KEYS (MEXC)
    botA: { 
        apiKey: process.env.MEXC_BOT_A_API_KEY || '',
        secret: process.env.MEXC_BOT_A_SECRET || '',
    },
    botB: { 
        apiKey: process.env.MEXC_BOT_B_API_KEY || '',
        secret: process.env.MEXC_BOT_B_SECRET || '',
    },
    ...
    database: {
        ...
        database: process.env.DB_NAME || process.env.MEXC_DB_NAME || 'marketcap',
...
async function flushPendingDbWrites() {
    ...
    while (pendingDbWrites.systemLogs.length > 0) {
        ...
        try {
            await dbPool.execute(
                `INSERT INTO mexc_system_logs (log_level, message, is_dry_run) VALUES (?, ?, ?)`,
                [item.level?.toUpperCase?.() || String(item.level || 'INFO'), String(item.message || ''), CONFIG.dryRun ? 1 : 0]
            );
        } catch (e) {
            console.error('DB Flush Error (mexc_system_logs):', e.message);
            break;
        }
    }
    ...
    while (pendingDbWrites.tradeHistory.length > 0) {
        ...
        try {
            ...
            console.error('DB Flush Error (mexc_trade_history):', e.message);
            break;
        }
    }
    ...
    while (pendingDbWrites.inventory.length > 0) {
        ...
        try {
            ...
            console.error('DB Flush Error (mexc_inventory_snapshot):', e.message);
            break;
        }
    }
}
...
async function initDatabase() {
    try {
        ...
        await dbPool.execute(`
            CREATE TABLE IF NOT EXISTS mexc_trade_history (
        ...
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN trend_progress DECIMAL(5, 2) DEFAULT 0 AFTER total_usd`); } catch (_) {}
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN taker_price DECIMAL(20, 8) DEFAULT NULL AFTER taker_order_status`); } catch (_) {}
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN taker_amount DECIMAL(20, 8) DEFAULT NULL AFTER taker_price`); } catch (_) {}
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN taker_total_usd DECIMAL(20, 8) DEFAULT NULL AFTER taker_amount`); } catch (_) {}
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN is_dry_run BOOLEAN DEFAULT FALSE`); } catch (_) {}
        try { await dbPool.execute(`ALTER TABLE mexc_trade_history ADD COLUMN execution_time_ms INT`); } catch (_) {}

        await dbPool.execute(`
            CREATE TABLE IF NOT EXISTS mexc_inventory_snapshot (
                id INT AUTO_INCREMENT PRIMARY KEY,
                timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
        ...
        await dbPool.execute(`
            CREATE TABLE IF NOT EXISTS mexc_system_logs (
                id INT AUTO_INCREMENT PRIMARY KEY,
                timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
...
async function logTradeHistory(tradeData, updateExisting = false) {
    ...
    try {
        if (updateExisting && tradeData.makerOrderId) {
            ...
            await dbPool.execute(`
                INSERT INTO mexc_trade_history (
        ...
        console.error('DB Log Error (mexc_trade_history):', e.message);
        broadcastLog(`❌ DB Log Error (mexc_trade_history): ${e.message}`, 'warn');
        if (dbPool) {
             try { await dbPool.execute(`INSERT INTO mexc_system_logs (log_level, message, is_dry_run) VALUES (?, ?, ?)`, ['ERROR', `DB Log Error: ${e.message}`, CONFIG.dryRun ? 1 : 0]); } catch(_) {}
        }
    }
}
...
async function logSystemLog(level, message) {
    ...
    try {
        await dbPool.execute(`INSERT INTO mexc_system_logs (log_level, message, is_dry_run) VALUES (?, ?, ?)`, [
            (level && typeof level === 'string') ? level.toUpperCase() : level,
            message,
            CONFIG.dryRun ? 1 : 0
        ]);
    } catch (e) {
        console.error('DB Log Error (mexc_system_logs):', e.message);
    }
}
...
async function logInventorySnapshot(botABal, botBBal, netChange) {
    ...
    try {
        await dbPool.execute(
            `INSERT INTO mexc_inventory_snapshot (bot_a_usdt, bot_a_token, bot_b_usdt, bot_b_token, net_token_change, is_dry_run) VALUES (?,?,?,?,?,?)`,
            [botABal.usdt, botABal.l1x, botBBal.usdt, botBBal.l1x, netChange, CONFIG.dryRun ? 1 : 0]
        );
    } catch (e) {
        console.error('DB Log Error (mexc_inventory_snapshot):', e.message);
    }
}
        ...
        for (const r of rows) {
            try {
                ...
                if (makerNew !== r.maker_order_status || takerNew !== r.taker_order_status) {
                    await dbPool.execute(
                        `UPDATE mexc_trade_history SET maker_order_status = ?, taker_order_status = ? WHERE id = ?`,
...
// ============================================================
// 6. MAIN ENGINE (STRICT MEXC VERSION)
// ============================================================
async function runLiveEngine() {
    broadcastLog("🚀 STARTING STRICT PURE VOLUME ENGINE (MEXC)...", 'req');
    await initDatabase();
    isRunning = true;
    ...
    try {
        ...
        botA = new ccxt.mexc({ apiKey: CONFIG.botA.apiKey, secret: CONFIG.botA.secret, ...exOpt });
        botB = new ccxt.mexc({ apiKey: CONFIG.botB.apiKey, secret: CONFIG.botB.secret, ...exOpt });
    ...
    const rawSymbol = CONFIG.pair; // MEXC handles L1X/USDT correctly
    ...
    while (isRunning && stats.volume < volumeTarget && Date.now() < timeLimit) {
        try {
            ...
            // REAL EXECUTION (Adapted for MEXC)
            let makerOrderId = null;
            const makerStartTime = Date.now();
            try {
                // MEXC uses unified createOrder
...
async function cancelAllOrders() {
    ...
    try {
        ...
        // call, which is what tripped MEXC's Cloudflare rate-limit and crashed.
        const a = botA || new ccxt.mexc({ apiKey: CONFIG.botA.apiKey, secret: CONFIG.botA.secret, enableRateLimit: true });
        const b = botB || new ccxt.mexc({ apiKey: CONFIG.botB.apiKey, secret: CONFIG.botB.secret, enableRateLimit: true });
...
(async function startServer() {
    await initDatabase();
    server.listen(5003, () => {
        console.log('✅ MEXC PURE VOLUME BOT RUNNING ON PORT 5003');
    });
})();
