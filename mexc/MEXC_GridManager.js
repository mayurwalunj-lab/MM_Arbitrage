...
const GRID_REFRESH_FLAG = path.join(__dirname, '..', 'arb', 'state', 'grid_refreshing_mexc.flag');
...
const GRID_CONFIG = {
    ...
    bot: {
        apiKey: process.env.MEXC_GRID_API_KEY || '',
        secret: process.env.MEXC_GRID_SECRET || '',
        // MEXC does not use UID
    },
    ...
    priceRangeGrids: {
        ...
        sell: [
            ...
            { minPrice: 11.0, maxPrice: 15.0, totalValue: 50, ordersPerRange:2, spacing: 'medium' },
            { minPrice: 10.0, maxPrice: 11.0, totalValue: 50, ordersPerRange:2, spacing: 'medium' },
            { minPrice: 8.9, maxPrice: 10.0, totalValue: 100, ordersPerRange:2, spacing: 'medium' },
            { minPrice: 8.55, maxPrice: 8.585, totalValue: 100, ordersPerRange: 5, spacing: 'medium' }, 
            { minPrice: 8.54, maxPrice: 8.55, totalValue: 350, ordersPerRange: 7, spacing: 'medium' },  
            { minPrice: 8.53, maxPrice: 8.54, totalValue: 300, ordersPerRange: 7, spacing: 'medium' },  
        ],
        ...
        buy: [
            ...
            { minPrice: 8.46, maxPrice: 8.47, totalValue: 80, ordersPerRange: 5, spacing: 'medium' },  
            { minPrice: 8.45, maxPrice: 8.46, totalValue: 50, ordersPerRange: 4, spacing: 'medium' },  
            { minPrice: 8.4447, maxPrice: 8.45, totalValue: 10, ordersPerRange: 2, spacing: 'medium' },                 // Below $8: Smaller defensive liquidity (wider spacing)
            { minPrice: 7.5, maxPrice: 8.4447, totalValue: 10, ordersPerRange: 1, spacing: 'wide' },
            // { minPrice: 7.0, maxPrice: 7.5, totalValue: 25, ordersPerRange: 3, spacing: 'wide' },
            // { minPrice: 7.0, maxPrice: 7.5, totalValue: 30, ordersPerRange: 1, spacing: 'very_wide' }
        ]
    },
    ...
    orderOperationTimeWindow: 300, // 5 minutes = 300 seconds
...
const GRID_ORDER_PREFIX = 'grid_';

function isGridOrder(order) {
    const cid = order.clientOrderId || order.info?.custom_id;
    return typeof cid === 'string' && cid.startsWith(GRID_ORDER_PREFIX);
}

function generateGridClientOrderId() {
    return GRID_ORDER_PREFIX + Date.now() + '_' + Math.random().toString(36).slice(2, 10);
}
    ...
    try {
        ...
        try {
            await dbPool.execute(`
                CREATE TABLE IF NOT EXISTS mexc_grid_log (
            ...
            `);
            try {
                await dbPool.execute(`CREATE INDEX idx_grid_log_created_at ON mexc_grid_log(created_at DESC)`);
                await dbPool.execute(`CREATE INDEX idx_grid_log_action ON mexc_grid_log(action)`);
                await dbPool.execute(`CREATE INDEX idx_grid_log_order_id ON mexc_grid_log(order_id)`);
            } catch (e) { /* Indexes may exist */ }
            log("📊 mexc_grid_log table ready.", 'info');
            try {
                await dbPool.execute(`
                    INSERT INTO mexc_grid_log (action, center_price, buy_budget_used, sell_budget_used, active_buy_orders, active_sell_orders, bot_name, is_dry_run)                    VALUES ('START', 0, 0, 0, 0, 0, 'gridBot', ${GRID_CONFIG.dryRun ? 1 : 0})
                `);
                log("📊 DB test write OK — check mexc_grid_log in database.", 'info');
            } catch (testErr) {
                log(`❌ DB test write failed: ${testErr.message}`, 'warn');
            }
        } catch (tableErr) {
            log(`❌ mexc_grid_log table setup failed: ${tableErr.message}`, 'warn');
            dbPool = null;
        }
    } catch (error) {
...
async function detectAndLogFilledOrders(openGridOrderIds) {
    ...
    try {
        const [rows] = await dbPool.execute(`
            ...
            FROM mexc_grid_log e1
            WHERE e1.action = 'PLACED' AND e1.order_id IS NOT NULL
            AND NOT EXISTS (
                SELECT 1 FROM mexc_grid_log e2
        ...
        for (let i = 0; i < maxReplacements; i++) {
            ...
            if (i < targetBuyOrders.length) {
                ...
                try {
                    ...
                    activeBuyOrders++;
                    log(`✅ Buy ${i+1}/${targetBuyOrders.length}: $${orderSizeUsdt.toFixed(2)} @ $${newOrder.price} [${newOrder.range.minPrice}-${newOrder.range.maxPrice}]`, 'info');
                    await logGridOrderEvent({
                        orderId: placed.id,
                        clientOrderId: cid,
                        side: 'buy',
                    ...
                    log(`❌ Failed to place buy order ${i+1} @ $${newOrder.price}: ${e.message}`, 'warn');
                    await delay(delayPerOperation); // Still wait even on error
                }
            }
        }
        ...
        for (let i = 0; i < maxSellReplacements; i++) {
            ...
            if (i < targetSellOrders.length) {
                ...
                try {
                    ...
                    activeSellOrders++;
                    log(`✅ Sell ${i+1}/${targetSellOrders.length}: $${orderSizeUsdt.toFixed(2)} @ $${newOrder.price} [${newOrder.range.minPrice}-${newOrder.range.maxPrice}]`, 'info');
                    await logGridOrderEvent({
                        orderId: placed.id,
                        clientOrderId: cid,
                        side: 'sell',
                    ...
                    log(`❌ Failed to place sell order ${i+1} @ $${newOrder.price}: ${e.message}`, 'warn');
                    await delay(delayPerOperation); // Still wait even on error
                }
            }
        }
